...
不需要同步完整聊天记录。不依赖额外云服务。只用很小的 repo-local markdown 文件，让任何 coding agent 都能读懂。
...
### Codex Marketplace 示例

这个仓库提供了一个本地 marketplace 示例：

```text
examples/marketplace.json
```

你可以复制或改造成个人/团队 marketplace。关键点是 marketplace entry 指向这个插件的本地 checkout：
...
- [`examples/marketplace.json`](examples/marketplace.json): Codex plugin marketplace 示例
- [`docs/PROMOTION.md`](docs/PROMOTION.md): GitHub、X、Hacker News、Reddit 和中文社区宣传文案
